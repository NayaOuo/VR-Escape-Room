    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit, Mathf.Infinity, layerMask))
        {
            ...
            if (ViveInput.GetPressDown(HandRole.LeftHand, ControllerButton.Grip))
            {
                ...
                {
                    selectedObject[0].GetComponent<NewBehaviourScript>().tl = !(selectedObject[0].GetComponent<NewBehaviourScript>().tl);
                    selectedObject[0].GetComponent<NewBehaviourScript>().hitl = hit;
                }

            }
            else if (ViveInput.GetPressDown(HandRole.LeftHand, ControllerButton.Trigger ))
            {
                if (selectedObject[0].GetComponent<NewBehaviourScript>().tl)
                {
